...
import {
  QrCode, Plus, Search, Pencil, Trash2, Eye, Filter, X, Calendar, ChevronDown,
...
const Ic = {
  qr: (s: number, c: string) => <svg width={s} height={s} viewBox="0 0 20 20" fill="none"><rect x="2" y="2" width="7" height="7" rx="1" stroke={c} strokeWidth="1.4"/><rect x="11" y="2" width="7" height="7" rx="1" stroke={c} strokeWidth="1.4"/><rect x="2" y="11" width="7" height="7" rx="1" stroke={c} strokeWidth="1.4"/><rect x="13" y="13" width="3" height="3" rx=".5" fill={c}/></svg>,
};

/* ═══ COLUMNS ═══ */
const ALL_COLUMNS = [
  { id: "codigo", label: "ID Codigo", fixed: true, width: "20%" },
  { id: "nome", label: "ID Nome", width: "25%" },
  { id: "empresa", label: "Empresa", width: "auto" },
  { id: "acoes", label: "Acoes", fixed: true, width: "100px", align: "right" as const },
];
...
export function QrCodesPage() {
  ...
  const activeFilters = (filterEmpresa ? 1 : 0);

  const empresas = [...new Set((qrCodes || []).map((qr: any) => qr.company?.name).filter(Boolean))].sort();

  const filtered = (qrCodes || []).filter((qr: any) => {
    const q = search.toLowerCase();
    const matchSearch = !q || qr.id_codigo.toLowerCase().includes(q) || qr.id_nome.toLowerCase().includes(q);
    const matchEmpresa = !filterEmpresa || qr.company?.name === filterEmpresa;
  ...
  const toggleAll = () => { if (selected.size === paged.length) setSelected(new Set()); else setSelected(new Set(paged.map((q: any) => q.id))); };
  ...
  const heroStats = [
    { label: "Total", value: qrCodes?.length || 0, color: C.azul },
  ];
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", color: C.textBody }} className="space-y-5">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0">
              <h2 style={{ fontSize: 22, fontWeight: 800, color: "#fff", lineHeight: 1.2, margin: 0 }}>
                QR <span style={{ color: C.primary }}>Codes</span>
              </h2>
      ...
      <div style={{
        ...
        borderRadius: cardRadius, boxShadow: dark ? "none" : "0 1px 3px rgba(0,0,0,0.04)",
      }}>
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10, padding: "12px 16px" }}>
            ...
            {showFilters && (
              <div style={{
                ...
                <div style={{ padding: "12px 16px" }}>
                  <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: C.textMuted, marginBottom: 8 }}>Empresa</p>
                  <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                    <button onClick={() => { setFilterEmpresa(""); setPage(1); }} style={{
                      display: "flex", alignItems: "center", gap: 8, padding: "6px 8px", borderRadius: 6,
                      border: "none", background: !filterEmpresa ? `${C.primary}12` : "transparent",
                      color: !filterEmpresa ? C.primary : C.textBody, fontSize: 11, fontWeight: !filterEmpresa ? 700 : 400,
                      cursor: "pointer", textAlign: "left", width: "100%",
                    }}>Todas as empresas</button>
                    {empresas.map(emp => (
                      <button key={emp} onClick={() => { setFilterEmpresa(emp); setPage(1); }} style={{
                        display: "flex", alignItems: "center", gap: 8, padding: "6px 8px", borderRadius: 6,
                        border: "none", background: filterEmpresa === emp ? `${C.primary}12` : "transparent",
                        color: filterEmpresa === emp ? C.primary : C.textBody, fontSize: 11, fontWeight: filterEmpresa === emp ? 700 : 400,
                        cursor: "pointer", textAlign: "left", width: "100%",
                      }}>{emp}</button>
                    ))}
                  </div>
          ...
          <div ref={periodoRef} style={{ position: "relative" }}>
            <button onClick={() => setShowPeriodo(!showPeriodo)} style={{
              ...
              background: "transparent", cursor: "pointer", transition: "all .15s", color: C.textBody,
            }}>
            ...
            {showPeriodo && (
              <div style={{
                ...
                background: C.cardBg, border: `1px solid ${C.cardBorder}`,
                boxShadow: "0 12px 36px rgba(0,0,0,0.18)", padding: "6px 0",
              }}>
                {periodoOptions.map(opt => {
                  const isActive = periodo === opt;
                  return (
                    <div key={opt} onClick={() => { setPeriodo(opt); setShowPeriodo(false); }} style={{
                      display: "flex", alignItems: "center", gap: 8,
                      padding: "8px 14px", fontSize: 11, cursor: "pointer",
                      fontWeight: isActive ? 700 : 500, color: isActive ? C.primary : C.textBody,
                      background: isActive ? `${C.primary}12` : "transparent", transition: "all .1s",
                    }}>
                      <div style={{ width: 14, height: 14, borderRadius: "50%", border: `1.5px solid ${isActive ? C.primary : C.cardBorder}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          {/* Export buttons */}
          <button title="Exportar Excel" style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <QrCode className="h-[22px] w-[22px]" style={{ color: C.primary }} />
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: C.textTitle, margin: 0 }}>QR Codes</h3>
            <p style={{ fontSize: 11, color: C.textMuted, margin: "3px 0 0" }}>
              {totalCount} {totalCount === 1 ? "registro" : "registros"} {search ? "filtrados" : "no total"} · Atualizado agora
            </p>
          </div>

          <div style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
            {search && (
              <span style={{ padding: "4px 10px", borderRadius: 12, fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.5px", background: `${C.primary}12`, color: C.primary }}>Filtrado</span>
            )}
            <div style={{
              ...
              <button onClick={() => setView("table")} style={{
                display: "inline-flex", alignItems: "center", gap: 6, padding: "5px 10px", borderRadius: 6, fontSize: 11, fontWeight: 600, border: "none",
                background: view === "table" ? (dark ? C.cardBg : "#fff") : "transparent", color: view === "table" ? C.primary : C.textMuted,
                boxShadow: view === "table" ? "0 1px 2px rgba(0,0,0,0.08)" : "none", cursor: "pointer",
              }}><List className="h-3 w-3" /> Tabela</button>
              <button onClick={() => setView("cards")} style={{
                display: "inline-flex", alignItems: "center", gap: 6, padding: "5px 10px", borderRadius: 6, fontSize: 11, fontWeight: 600, border: "none",
                background: view === "cards" ? (dark ? C.cardBg : "#fff") : "transparent", color: view === "cards" ? C.primary : C.textMuted,
                boxShadow: view === "cards" ? "0 1px 2px rgba(0,0,0,0.08)" : "none", cursor: "pointer",
              }}><Grid3x3 className="h-3 w-3" /> Cards</button>
            </div>
            ...
            <div ref={configRef} style={{ position: "relative" }}>
              ...
              {showConfig && (
                <div style={{
                  position: "absolute", right: 0, top: "calc(100% + 6px)", zIndex: 50, width: 300, borderRadius: cardRadius, overflow: "hidden",
                  background: C.cardBg, border: `1px solid ${C.cardBorder}`, boxShadow: "0 12px 36px rgba(0,0,0,0.18)",
                }}>
                  ...
                  <div style={{ display: "flex", borderBottom: `1px solid ${C.cardBorder}`, background: dark ? "rgba(255,255,255,0.02)" : C.surfaceSoft }}>
                    {([{ id: "colunas" as const, label: "Colunas", icon: Columns3 }, { id: "densidade" as const, label: "Densidade", icon: Rows3 }, { id: "aparencia" as const, label: "Aparencia", icon: LayoutGrid }]).map(t => (
                      <button key={t.id} onClick={() => setConfigTab(t.id)} style={{
                        flex: 1, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "9px 8px", fontSize: 11, fontWeight: 600, border: "none", background: "transparent", cursor: "pointer",
                        color: configTab === t.id ? C.primary : C.textMuted, borderBottom: `2px solid ${configTab === t.id ? C.primary : "transparent"}`,
                  ...
                  <div style={{ maxHeight: 320, overflowY: "auto", padding: "12px 16px" }}>
                    {configTab === "colunas" && ALL_COLUMNS.map(col => {
                      ...
                      return (
                        <div key={col.id} onClick={() => { if (!col.fixed) { const n = new Set(visibleCols); vis ? n.delete(col.id) : n.add(col.id); setVisibleCols(n); } }}
                          style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 8px", borderRadius: 6, cursor: col.fixed ? "not-allowed" : "pointer", opacity: col.fixed ? 0.6 : 1 }}>
                          <GripVertical className="h-2.5 w-2.5" style={{ color: C.textMuted }} />
                          <div style={{ width: 14, height: 14, borderRadius: 3, border: `1.5px solid ${vis ? C.primary : C.textMuted}`, background: vis ? C.primary : "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    ...
                    {configTab === "densidade" && ([
                      { id: "compact" as const, label: "Compacta", desc: "30px" }, { id: "normal" as const, label: "Normal", desc: "42px" }, { id: "comfortable" as const, label: "Confortavel", desc: "56px" },
                    ]).map(opt => (
                      <div key={opt.id} onClick={() => setDensity(opt.id)} style={{
                        ...
                        border: `1.5px solid ${density === opt.id ? C.primary : C.cardBorder}`, background: density === opt.id ? `${C.primary}08` : "transparent", cursor: "pointer",
                      }}>
                        <div style={{ width: 18, height: 18, borderRadius: "50%", border: `2px solid ${density === opt.id ? C.primary : C.textMuted}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                          {density === opt.id && <div style={{ width: 8, height: 8, borderRadius: "50%", background: C.primary }} />}
                        </div>
                    ...
                    {configTab === "aparencia" && ([
                      { id: "zebra" as const, label: "Linhas zebradas", desc: "Alterna fundo" }, { id: "verticalBorders" as const, label: "Bordas verticais", desc: "Linhas entre colunas" },
                    ]).map(opt => (
                      <div key={opt.id} onClick={() => setAppearance(a => ({ ...a, [opt.id]: !a[opt.id] }))} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px", borderRadius: 6, cursor: "pointer" }}>
                        ...
                        <div style={{ width: 30, height: 18, borderRadius: 9, padding: 2, background: appearance[opt.id] ? C.primary : (dark ? C.cardBorder : "#CBD5E1"), transition: "background .2s" }}>
                          <div style={{ width: 14, height: 14, borderRadius: "50%", background: "#fff", boxShadow: "0 1px 3px rgba(0,0,0,0.15)", transition: "transform .2s", transform: appearance[opt.id] ? "translateX(12px)" : "translateX(0)" }} />
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {isLoading ? (
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
                ...
                <tr style={{ background: dark ? "rgba(255,255,255,0.03)" : C.surfaceSoft }}>
                  ...
                  {cols.map(col => (
                    <th key={col.id} onClick={() => handleSort(col.id)} style={{
                      padding: `4px ${D.padX}px`, width: col.width, textAlign: col.align || "left",
                      ...
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                        {col.label}
                        {sortCol === col.id
                          ? <ChevronUp className="h-2.5 w-2.5" style={{ color: C.primary, transform: sortDir === "desc" ? "rotate(180deg)" : "none", transition: "transform .15s" }} />
                          : col.id !== "acoes" ? <ChevronsUpDown className="h-2.5 w-2.5" style={{ color: C.textLight }} /> : null
                        }
                      </span>
                    </th>
                  ))}
              ...
              <tbody>
                {paged.map((qr: any, idx: number) => {
                  const isSel = selected.has(qr.id);
                  return (
                    <tr key={qr.id} style={{
                    ...
                    >
                      <td style={{ width: 36, padding: `${D.padY}px ${D.padX}px` }}>
                        <div onClick={() => toggleSel(qr.id)} style={{ width: 14, height: 14, borderRadius: 3, cursor: "pointer", border: `1.5px solid ${isSel ? C.primary : C.textMuted}`, background: isSel ? C.primary : "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          {isSel && <Check className="h-2.5 w-2.5" style={{ color: "#1a1a1a" }} />}
                        </div>
                      </td>
                      {visibleCols.has("codigo") && <td style={{ padding: `${D.padY}px ${D.padX}px`, fontWeight: 700, color: C.primary, fontSize: D.fs }}>{qr.id_codigo}</td>}
                      {visibleCols.has("nome") && <td style={{ padding: `${D.padY}px ${D.padX}px`, fontSize: D.fs, color: C.textMuted }}>{qr.id_nome}</td>}
                      {visibleCols.has("empresa") && <td style={{ padding: `${D.padY}px ${D.padX}px`, fontSize: D.fs, fontWeight: 600, color: C.textBody }}>{qr.company?.name || "—"}</td>}
                      {visibleCols.has("acoes") && (
                        <td style={{ padding: `${D.padY}px ${D.padX}px`, textAlign: "right" }}>
                          <div style={{ display: "flex", justifyContent: "flex-end", gap: 2 }}>
                            <button onClick={() => setPreviewItem(qr)} style={{ width: 28, height: 28, borderRadius: 6, border: "none", background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: C.azul }} title="Ver">
                              <Eye className="h-3.5 w-3.5" />
                            </button>
                            <button onClick={() => setEditingItem(qr)} style={{ width: 28, height: 28, borderRadius: 6, border: "none", background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: C.verde }} title="Editar">
                              <Pencil className="h-3.5 w-3.5" />
                            </button>
                            <button onClick={() => setDeleteItem(qr)} style={{ width: 28, height: 28, borderRadius: 6, border: "none", background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: C.vermelho }} title="Excluir">
                ...
                {paged.length === 0 && (
                  <tr><td colSpan={cols.length + 1} style={{ padding: "48px 16px", textAlign: "center", color: C.textMuted }}>
                    ...
                    <p style={{ fontSize: 13, fontWeight: 600 }}>Nenhum QR Code encontrado</p>
                  </td></tr>
                )}
              </tbody>
            </table>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 12, padding: 20 }}>
            {paged.map((qr: any) => (
              <div key={qr.id} style={{ padding: 16, borderRadius: 10, border: `1px solid ${C.cardBorder}`, background: dark ? "rgba(255,255,255,0.03)" : "#fafbfc" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <span style={{ fontWeight: 700, color: C.primary, fontSize: 14 }}>{qr.id_codigo}</span>
                  <div style={{ display: "flex", gap: 2 }}>
                    <button onClick={() => setPreviewItem(qr)} style={{ width: 26, height: 26, borderRadius: 6, border: "none", background: "transparent", cursor: "pointer", color: C.azul, display: "flex", alignItems: "center", justifyContent: "center" }}><Eye className="h-3.5 w-3.5" /></button>
                    <button onClick={() => setEditingItem(qr)} style={{ width: 26, height: 26, borderRadius: 6, border: "none", background: "transparent", cursor: "pointer", color: C.verde, display: "flex", alignItems: "center", justifyContent: "center" }}><Pencil className="h-3.5 w-3.5" /></button>
                    <button onClick={() => setDeleteItem(qr)} style={{ width: 26, height: 26, borderRadius: 6, border: "none", background: "transparent", cursor: "pointer", color: C.vermelho, display: "flex", alignItems: "center", justifyContent: "center" }}><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                </div>
                <p style={{ fontSize: 12, color: C.textMuted }}>{qr.id_nome}</p>
                <p style={{ fontSize: 11, fontWeight: 600, color: C.textBody, marginTop: 4 }}>{qr.company?.name || "—"}</p>
              </div>
            ))}
            {paged.length === 0 && <div style={{ gridColumn: "1 / -1", padding: "48px 16px", textAlign: "center", color: C.textMuted }}><Inbox className="h-8 w-8 mx-auto mb-2" style={{ color: C.textLight }} /><p>Nenhum QR Code encontrado</p></div>}
          </div>
        )}
        ...
        {totalPages > 1 && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px", borderTop: `1px solid ${dark ? "rgba(255,255,255,0.06)" : C.cardBorder}` }}>
            <span style={{ fontSize: 11, color: C.textMuted }}>
              Pagina {page} de {totalPages} ({totalCount} registros){selected.size > 0 && ` · ${selected.size} selecionados`}
            </span>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <button disabled={page <= 1} onClick={() => setPage(page - 1)} style={{ ...btnBase(false), opacity: page <= 1 ? 0.4 : 1, cursor: page <= 1 ? "not-allowed" : "pointer" }}>
                <ChevronLeft className="h-3.5 w-3.5" /> Anterior
              </button>
              <span style={{ fontSize: 12, fontWeight: 700, color: C.primary, padding: "0 8px" }}>{page}</span>
              <button disabled={page >= totalPages} onClick={() => setPage(page + 1)} style={{ ...btnBase(false), opacity: page >= totalPages ? 0.4 : 1, cursor: page >= totalPages ? "not-allowed" : "pointer" }}>
      ...
      <QrCodeForm open={!!editingItem} onOpenChange={open => !open && setEditingItem(null)} initialData={editingItem ? { company: editingItem.company?.name || "", idCodigo: editingItem.id_codigo, idNome: editingItem.id_nome, qrCodeId: editingItem.id } : null} onSubmit={handleEdit} />
      <ConfirmDialog open={!!deleteItem} onOpenChange={open => !open && setDeleteItem(null)} title="Excluir QR Code" description={`Tem certeza que deseja excluir ${deleteItem?.id_codigo}?`} onConfirm={handleDelete} />
      <QrCodePreviewModal open={!!previewItem} onOpenChange={open => !open && setPreviewItem(null)} qrData={previewItem ? { idCodigo: previewItem.id_codigo, idNome: previewItem.id_nome, company: previewItem.company?.name, address: previewItem.company?.address } : null} />
    </div>
  );
}
